...
x = np.linspace(0, 100000000000)
y1 = 768 * 2 * x  # 绘制二次函数 y = x^2
y2 = 150 * 2 * x + 4096 * 2 * 200
# y3 = 150 * 2 * x + 4096 * 2 * 200 + 5 * (2 * 8000000000 * 2000)
y3 = 150 * 2 * x + 4096 * 2 * 200 + 5 * (32 * (8 * 4096 * 4096 + 6 * 4096 * 14336) + 2 * 4096 * 128256) * 576
...
ax.plot(x, y1, label='CLIP', color=colors[4], linestyle='-', linewidth=3, markersize=8)
ax.plot(x, y3, label='three pipelines', color=colors[5], linestyle='-', linewidth=3, markersize=8)
...
plt.text(0.3, 0.3,  # 坐标位置 (图形坐标，0-1之间)
         r'$f_2(x)=768 \times 2 \times x$',  # LaTeX公式
         transform=plt.gca().transAxes,  # 使用坐标轴坐标
         fontsize=20,
         bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
plt.text(0, 0.65,  # 坐标位置 (图形坐标，0-1之间)
         r'$f_1(x)=150\times2\times x+4096\times 2\times200+5\times2\times8B\times2000$',  # LaTeX公式
         transform=plt.gca().transAxes,  # 使用坐标轴坐标
         fontsize=18,
         bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))

...
def main():
    ...
    with torch.no_grad(), torch.cuda.amp.autocast() if training_args.fp16 else nullcontext():
        for batch_idx, (texts, imgs_path, text_ids, img_ids) in tqdm(enumerate(test_dataloader),
            ...
            for id, rep in zip(batch_ids, reps):
                id_to_dense_reps[id] = rep.cpu().detach().float().numpy()
            ...
            for indice in range(len(batch_ids)):
                ...
                disassemble_logit = disassemble_logits[
                                    indice * len(llama3_retrieval_disassemble_text_prompts):(
                                        ...
                                        llama3_retrieval_disassemble_text_prompts)]
                if model_args.eol_type == 'disassembleeol_concrete':
                    id_to_logit[id] = query_logits[indice].cpu().detach().float().numpy()
                else:
                    id_to_logit[id] = disassemble_logit.cpu().detach().float().numpy()
    ...
    for i in range(max(len(dense_retriever_indices), len(sparse_retriever_indices))):
        ...
        for id, rep in tqdm(id_to_dense_reps.items()):
            if dense_retriever is not None:
                if isinstance(rep, list):
                    for qid, reps in zip(batch_ids, rep):
                    ...
                    rep = np.array([rep])
                    dense_scores, dense_rankings = search_queries(dense_retriever, rep, look_up,
                                                                  search_args)
                    dense_run.update(
                        get_run_dict([id], dense_scores, dense_rankings, search_args.remove_query))
        ...
        for id, logit in tqdm(id_to_logit.items()):
            if sparse_retriever is not None:
                ...
                if 'disassembleeol' in model_args.eol_type:
                    ...
                    if model_args.eol_type == 'disassembleeol_concrete':
                        tokens, values = get_img_valid_disassemble_tokens_values(processor,
                                                                                 torch.tensor(logit).to(device),
                                                                                 vocab_dict,
                                                                                 data_args,
                                                                                 filtered_ids, torch.tensor(logit).to(device),
                                                                                 model_args)
                    else:
                        tokens, values = get_img_valid_disassemble_tokens_values(processor,
                                                                                 torch.tensor(logit).to(device),
                ...
                sparse_scores, sparse_rankings = sparse_search(sparse_retriever, batch_topics,
                                                               ...
                                                               search_args)
                sparse_run.update(
                    get_run_dict([id], sparse_scores, sparse_rankings, search_args.remove_query))
